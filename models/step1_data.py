...
class CloverDataLoader:
    ...
    def __init__(self, experiment_dir: str):
        ...
        # 数据存储
        self.reads: List[str] = []
        self.clover_labels: List[int] = []  # Clover聚类结果
    ...
    def _load_all_data(self):
        ...
        # 1. 加载FedDNA格式的reads和Clover标签
        self.reads, self.clover_labels = self._load_feddna_format()
        ...
        print(f"   - Clover簇数: {len(set(self.clover_labels))}")
...
class Step1Dataset(Dataset):
    ...
    def __init__(self, data_loader: CloverDataLoader, max_len: int = 150):
        ...
        print(f"📊 数据集统计:")
        print(f"   - 有效reads: {len(self.valid_indices)}/{len(data_loader.reads)}")
        print(f"   - 过滤噪声: {len(data_loader.reads) - len(self.valid_indices)}")
    ...
    def __getitem__(self, idx):
        ...
        return {
            ...
            'clover_label': clover_label,   # Clover聚类标签
