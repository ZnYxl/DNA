...
def statistics_s(x, y):
    z = x.eq(y).int()
    acc = torch.all(z, dim=1).int()
    num = torch.sum(acc)
    return num
...
def ema_update(global_model, new_model, alpha=0.2):
    for param_old, param_new in zip(global_model.parameters(), new_model.parameters()):
        param_old.data = alpha * param_new.data + (1 - alpha) * param_old.data
