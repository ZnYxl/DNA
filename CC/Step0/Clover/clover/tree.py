...
class Trie:
                ...
                if num +2 < len_ :
                    for k in list :
                        if not node.children[k].children[dict[word[num+1]]]:
                            ...
                            fin_list.append(k)
                    return [num,fin_list] #[错误位置, 可漂移的分支列表]
                if num +2 == len_:
                    for k in list :
                        if not node.children[k].children[dict[word[num+1]]]:
                            pass
    ...
    def fuzz_fin(self,word,max_value):
        ...
        list=[[word,0]] #[当前待匹配的序列, 已经漂移的次数]
        fin_list=["",1000] #[匹配到的序列, 其漂移次数]
        dict2  = {}
        ...
        while True :
            ...
            if type(a) == int : #匹配成功比较编辑长度
                if dna[1] < fin_list[1] :
                    fin_list=[a,dna[1]]
            elif self.fuzz_align(word)[1] == [] : #没有可漂移方向，直接退出
                break
            elif a[0] == len(dna[0])-1: #错误点是最后一位
                for i in range(len(a[1])):
                    i=a[1][i]
                    k = dna[0][:a[0]]+dict2[i] #把原序列最后一个字符替换成候选碱基
                    list.append([k,dna[1]+1])                
            else: #错误点在中间
                for i in range(len(a[1])):
                    i=a[1][i]
                    k = dna[0][:a[0]]+dict2[i]+dna[0][a[0]-len(dna[0])+1:] #替换
                    list.append([k,dna[1]+1])
