...
class MyProcess(Process):
    ...
    def cluster(self,read):
        ...
        if dna_str_num < self.config_dict['read_len_min'] or "N" in dna_str:
            ...
            a_align = self.a_tree.fuzz_fin(dna_a_str,self.config_dict['tree_threshold']) #优先使用a_tree 模糊匹配
            if a_align[1]<self.fuzz_tree_nums:  #If the match is successful, it is recorded.
                if self.config_dict['Virtual_mode'] == False:
                    self.index_list.append((dna_index,self.ref_dict[a_align[0]][0])) #不是虚拟模式则加入index_list,并获取该簇的主标签
                else:
                    self.ref_dict[a_align[0]].append(dna_tag)#虚拟模式把tag存入列表（把 dna_tag append 到 ref_dict 对应簇 id 下）
                
                if self.align_swicth is True:  #If global comparison is done, global comparison is started after matching.
                    error_list = []
                    align_list=a_align
                    if self.config_dict["now_align_alg"] == True: #对所有匹配到的 read 都执行 global_align
                        error_list=ag.global_align(self.ref_list[align_list[0]],dna_str)
                    else:
                        if dna_str_num==self.read_len :#只在当前 read 的长度等于期望 read_len 且它与 core 不完全相同的时候才做 global_align
                    ...
                    for line in error_list:
                        # 旧：ref_error_dict[cid] = {"nums": int_total, pos:int_count, ...}
                        # 新：ref_error_dict[cid] = {
                        #   "nums": int_total_aligns_seen,
                        #   "pos_stats": {
                        #       pos0: {"A": nA, "C": nC, "G": nG, "T": nT, "total": n_total_pos0},
                        #       pos1: {...},
                        #       ...
                        #   }
                        # }
                        ...
                        if align_list[0] in self.ref_error_dict : 
                            ...
                            if self.ref_error_dict[align_list[0]][line[0]]/self.ref_error_dict[align_list[0]]["nums"] >0.5 and self.ref_error_dict[a_align[0]][line[0]]>5 :
                                ...
                                self.ref_error_dict[align_list[0]]={} #清空
            ...
            elif self.b_tree.fuzz_fin(dna_b_str,self.config_dict['tree_threshold'])[1] < self.fuzz_tree_nums:#匹配距离小于阀值则匹配成功
                ...
                if dna_str_num >= self.config_dict['read_len_min'] :
                    fin_align=["",1000]
                    for i in self.loc_nums: #在纵向漂移范围内循环
                        if self.h_index == 0 :#默认位置
                            dna_c_str=dna_str[self.fuzz_list[0]-i:self.fuzz_list[0]+self.fuzz_list[2]-i] #用 self.fuzz_list[2] 作为窗口长度
                        else:
                            ...
                            for line in error_list:
                                if align_list[0] in self.ref_error_dict : 
                                    ...
                                    if line[0] in  self.ref_error_dict[align_list[0]]:
                                        ...
                                        self.ref_error_dict[align_list[0]][line[0]]=1
                                    if self.ref_error_dict[align_list[0]][line[0]]/self.ref_error_dict[align_list[0]]["nums"] >0.5 and self.ref_error_dict[a_align[0]][line[0]]>5 :
                                        now_read=self.ref_list[align_list[0]][:line[0]]+line[1]+self.ref_list[align_list[0]][line[0]+1:]#修正代表序列

                                        self.a_tree.insert(now_read[:self.dna_tree_nums],align_list[0])#更新后的代表序列重新插入到 索引树 (a_tree)
                                        self.ref_list[align_list[0]]=now_read
    ...
    def run(self):
        ...
        len_=len(self.name) #后续代码用来确定当前处理的是哪一个数据集或者文件的名字
...
if __name__ == '__main__':
    ...
    if PROCESS_INDEX == 0 and config_dict['fast_mode'] == True : #如果进程数为 0 并且启用了快速模式 (fast_mode)，则仅启动一个进程
        N_PROCESS=1
        process_names=['all']
    ...
    elif PROCESS_INDEX == 0 and config_dict['fast_mode'] == False : #如果进程数为 0 并且没有启用快速模式，数据仍是单进程处理
        N_PROCESS=1
        process_names=['all']
        data_dict = {}
        data_dict['all']=[]
    elif PROCESS_INDEX > 0 : #如果进程数大于 0，使用多进程模式并根据碱基前缀划分任务
        N_PROCESS = 4**PROCESS_INDEX
        process_names = all_permutations(["A","T","G","C"],PROCESS_INDEX) #调用 all_permutations 函数生成 PROCESS_INDEX 长度的所有碱基排列，生成进程的名字


        
        st_pre = time.time() #返回当前时间的时间戳
        data_dict = {}.fromkeys(process_names) #每个进程创建一个列表
        for i in data_dict:
            data_dict[i] = []
        ...
        else: #如果没有启用快速模式，则不进行预处理，直接将数据分配到进程
            data_dict[i] = []
    ...
    for i in process_dict:
        process_dict[i] = MyProcess(i, data_dict[i], q_output) #字典中为每个进程名创建一个 MyProcess 实例

    for i in process_dict:
        process_dict[i].start() #启动每个进程，开始并行处理任务。每个进程都会执行其对应的 MyProcess 类中的 run() 方法，开始处理数据并将结果存入队列
    ...
    for i in process_dict:
        process_dict[i].join() #调用每个进程的 join() 方法，确保主进程在所有子进程完成之前不会退出。join() 会阻塞主进程，直到当前进程完成
    ...
    for key in process_dict :
        new_count_dict["sum_read_num"]+=count_dict[key+"sum_read_num"] #所有进程读取的总数
        new_count_dict["error_num"]+=count_dict[key+"error_num"] #所有进程中的错误数量
        new_count_dict["sum_cluster_num"]+=count_dict[key+"sum_cluster_num"]#所有进程中聚类的总数
        tag_list=count_dict[key+"sum_tag"] #统计所有进程中的标签，并确保每个标签只出现一次
