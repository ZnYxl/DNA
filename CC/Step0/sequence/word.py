...
class Error_stats:
    ...
    def output_word(self, error_dict=None):
...
def load_dict_from_file(file_path):
    """
    从文件中读取字符串格式的字典，并将其转换回Python字典对象。
    """
    print(f"正在从文件加载字典: {file_path}")
    with open(file_path, 'r', encoding='utf-8') as f:
        content = f.read()

    # 因为字典中包含 numpy array，使用 eval 是最直接的方式。
    # ast.literal_eval 更安全，但无法解析 numpy array。
    # 在这个场景下，文件内容是程序自己生成的，所以使用 eval 是安全的。

    # 为了让 eval 能够识别 'np.zeros' 或 'array'，我们需要导入 numpy
    # 注意：你的代码在保存时可能将 numpy array 存为 array(...) 的形式
...
if __name__ == '__main__':
    # --- 请在这里修改你的文件路径 ---
    # 1. 你的参考序列文件路径
    ref_path = "/mnt/st_data/linhaiyan/data-nbt17/id20.fasta"
    # 2. 你的SAM文件路径
    read_path = "/mnt/st_data/linhaiyan/data-nbt17/mem-se.sam"
    # 3. 你已经生成的字典文件路径
    dict_file_path = "error_count_file_DY5-2025-07-12-33_error_dict"  # <-- 修改为你的实际文件名
    # 4. 报告文件的前缀名
    report_name = 'error_count_file_DY5'
    # --- 修改结束 ---

    # 即使我们不重新处理数据，仍然需要初始化Error_stats类
    # __init__ 方法需要这些路径来计算一些初始值，比如 ref_len
    print("正在初始化 Error_stats 实例...")
    # 注意：这里的 name 参数应该与你的字典文件前缀匹配，这样生成的 Word 文档名才能一致。
    error_model = Error_stats(ref_path, read_path, name=report_name)
    print("实例初始化完成。")

    # 从文件加载已经统计好的错误字典
    error_dictionary = load_dict_from_file(dict_file_path)

    if error_dictionary:
        # 调用 output_word 方法，并直接传入加载的字典
        print("正在生成 Word 报告...")
        error_model.output_word(error_dict=error_dictionary)

        # 从字典中提取时间戳，或者直接使用文件名来构建最终的Word文件名
        # 你的代码会自动处理文件名，我们只需要确保 report_name 正确
        print(f"报告生成完毕！请检查名为 '{error_model.name}.docx' 的文件。")
