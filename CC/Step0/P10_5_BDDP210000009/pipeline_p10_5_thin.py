...
EXPERIMENT_DIR = '/mnt/st_data/liangxinyi/code/CC/Step0/Experiments/p10_5'
...
def step7_deploy(idx_map, clover_tag_path, ref_seqs):
    ...
    gt_tags_path = os.path.join(EXPERIMENT_DIR, 'p10_5_tags_reads.txt')
    ...
    gt_refs_path = os.path.join(EXPERIMENT_DIR, 'p10_5_refs.txt')
    ...
    print(f"       2>&1 | tee p10_5_ours_v1.log")
    print()
    print(f"  ⚠️  注意:")
    print(f"     --max_length 201    模型最大长度（保持 FedDNA 预训练默认）")
    print(f"     --ref_length {REF_LEN}    reads/MV 长度，FASTA 保存裁剪用 (v15 修复)")
    print(f"     GT ref 实际 {GT_REF_LEN}bp，主代码会按 read 长度对齐评估")
...
def main():
    ...
    print()
    print("=" * 60)
    print("  🚀  P10_5 (BDDP210000009) Pipeline（打薄版）")
    print("=" * 60)
    print()
