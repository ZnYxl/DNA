...
# GradHC 参数（完全使用 GradHC 默认值 —— 论文 SOTA 原始配置）
#   退回默认的原因：之前 q=8 / sd_high=0.40 是在旧的"每 tag≤30 条"独立打薄数据上调的，
#   换成全局随机打薄的同源数据后分布已变（簇大小分布不同），魔改参数不再适用，
#   且 GradHC 论文是用默认参数跑赢 Clover 的。本版退回默认作为可辩护的 SOTA 基线。
#   默认值（源码 __init__）：q=6, k=3, m=40, L=32, distance_threshold=12
#   sd_high 默认：clustering_given_chunk=0.32, final_clustering=0.25（均不覆盖）
GRADHC_Q    = 6
GRADHC_K    = 3
GRADHC_M    = 40
GRADHC_L    = 32
...
def step2_run_gradhc(gradhc_input_path):
    ...
    print(f"          sd_high=默认 (chunk=0.32 / final=0.25，不覆盖 —— SOTA 原始配置)")
...
def step7_deploy(gradhc_tag_path, keep_ratio):
    ...
    exp_rel = f"CC/Step0/Experiments/seq_1d_gradhc_default_{ratio_tag}"
...
def main():
    ...
    OUTPUT_DIR = os.path.join(BASE_DIR, f'gradhc_default_out_{ratio_tag}')
    EXPERIMENT_DIR = os.path.join(EXP_ROOT, f'seq_1d_gradhc_default_{ratio_tag}')
    ...
    print(f"  GradHC:           q={GRADHC_Q} k={GRADHC_K} m={GRADHC_M} L={GRADHC_L} "
          f"dist={GRADHC_DIST} sd_high=默认(SOTA原始配置)")
